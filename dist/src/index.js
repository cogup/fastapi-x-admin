...
class AdminRouters extends fastapi_1.MakeRouters {
    openAPISpec;
    openAPISpecString;
    onLoad(fastAPI) {
    ...
    async apiRouter(_request, reply) {
    ...
    async publicRouter(request, reply) {
        ...
        const filename = path === '/' || path === '/"index.html"'
            ? 'index.html'
            : path.replace('//', '');
        if (filename === 'index.html' ||
            !this.fileExists(`${ASSETS_DIR}/${filename}`)) {
            const file = this.loadFile(`${ASSETS_DIR}/index.html`);
            const content = file.content.replace('</head>', `<script>window.specification=${this.openAPISpecString}</script></head>`);
...
], AdminRouters.prototype, "apiRouter", null);
__decorate([
    (0, fastapi_1.Get)('/*'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object]),
